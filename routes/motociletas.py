...
bp = Blueprint('motociletas', __name__, url_prefix='/api/motociletas')


@bp.route('', methods=['GET'])
@require_auth
def listar_motociletas():
    try:
        motos = get_all("""
...
def crear_motocileta():
    try:
        ...
        id_moto = execute("""
            INSERT INTO Motociletas (id_cliente, id_modelo, placa, color, kilometraje, vin, num_serie, anio)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
...
def actualizar_motocileta(id):
    try:
        data = request.json
        execute("""
            UPDATE Motociletas SET id_cliente=?, id_modelo=?, placa=?,
